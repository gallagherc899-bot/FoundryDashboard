...
def load_and_prepare_data():
    ...
    try:
        ...
        df_ml[TARGET_COLUMN] = (df_ml['scrap_percent_hist'] > 0).astype(int)
...
# --- 3. MACHINE LEARNING MODEL (with single-sample class filtering fix) ---
...
def train_random_forest_model(df_ml: pd.DataFrame):
    ...
    if df_ml.empty:
        ...
        # Returning dummy empty data structures
    ...
    # Use .copy() to ensure we are working with a distinct structure if needed
    ...
    if single_sample_classes:
        ...
        # Use a temporary DataFrame for filtering
    ...
    # Train-test split (now safe from single-sample classes)
...
def predict_part_risk(model, part_id, df_ml, feature_cols):
    ...
    if part_data.empty or model is None:
        return 0.0, None
...
if model is None:
    # If model training failed, stop execution here after displaying the error in the function
    st.stop()
...
# Diagnostics (This section was duplicated, removing duplication and moving to the end of the script for consistency)
# This is now only present at the end of TAB 2, which is sufficient.

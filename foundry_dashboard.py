...
# Keys added to resolve StreamlitDuplicateElementId
MATERIAL_COST_PER_LB = st.sidebar.number_input(
    "Material Cost/Value per lb ($)", value=2.50, step=0.10, format="%.2f", key='material_cost'
)
LABOR_OVERHEAD_COST_PER_LB = st.sidebar.number_input(
    "Labor/Overhead Cost per lb ($)", value=0.50, step=0.10, format="%.2f", key='labor_cost'
)
AVG_NON_MATERIAL_COST_PER_FAILURE = st.sidebar.number_input(
    "Non-Material Cost per Failure ($)", value=150.00, step=10.00, key='failure_cost'
)

# Simulation Target and Effort (KEPT IN SIDEBAR)
st.sidebar.subheader("Target & Effort")
TARGET_SCRAP_PERCENT = st.sidebar.slider(
    "Ultimate Scrap Target (%)", min_value=0.5, max_value=5.0, value=1.5, step=0.1, key='scrap_target'
)
REDUCTION_SCENARIOS = [
...
with input_cols[1]:
    input_weight = st.number_input("B. Piece Weight (lbs)", value=df_avg.loc[df_avg['part_id'] == selected_part_id, 'Avg_Piece_Weight'].iloc[0].round(2), min_value=0.01, key='input_weight')
with input_cols[2]:
    input_quantity = st.number_input("C. Number of Pieces in Run", value=int(df_avg.loc[df_avg['part_id'] == selected_part_id, 'Avg_Order_Quantity'].iloc[0]), min_value=1, key='input_quantity')
with input_cols[3]:
    input_cost = st.number_input("D. Value/Cost of Single Finished Part ($)", value=25.00, min_value=0.01, key='input_cost')
...
for i, feature in enumerate(feature_cols):
    ...
    with feature_cols_display[col_index]:
        # Added unique key for defect checkboxes
...
with tuning_cols[0]:
    ...
    # Tuning Sliders (Moved from Sidebar) - Keys already exist here

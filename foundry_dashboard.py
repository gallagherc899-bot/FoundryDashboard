...
if st.button("Predict Scrap Risk"):
    ...
    if part_known:
        ...
        reliability_next_run = np.exp(-lambda_scrap)
        ...
        st.metric("Reliability Next Run", reliability_display)

    expected_scrap_count = round(predicted_proba * quantity)
    expected_loss = round(expected_scrap_count * cost_per_part, 2)
    st.subheader("💰 Financial Impact")
    st.write(f"Predicted scrap: {round(predicted_proba * 100, 2)}% for {quantity} parts.")
